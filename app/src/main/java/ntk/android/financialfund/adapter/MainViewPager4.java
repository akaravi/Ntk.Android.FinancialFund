...
public class MainViewPager4 extends PagerAdapter {
    ...
    private MainAdapter4 adapter;
    PanelInterface myInterface;
    int PageCount;

    public MainViewPager4(PanelInterface panelInterface, List<PanelViewModel>... models) {
        frislList = models[0];
        secondList = models[1];
    ...
    public Object instantiateItem(ViewGroup view, final int position) {
        View imageLayout = LayoutInflater.from(view.getContext()).inflate(R.layout.viewpager_main4, view, false);
        if (position == 0) {
            RecyclerView rv = imageLayout.findViewById(R.id.rv_parent);

            GridLayoutManager llm = new GridLayoutManager(view.getContext(), 2, GridLayoutManager.VERTICAL, false);
            try {
                adapter = new MainAdapter4(frislList, myInterface);
...
//                        3));
            ...
            GridLayoutManager llm = new GridLayoutManager(view.getContext(), 2, GridLayoutManager.VERTICAL, false);
            try {
                adapter = new MainAdapter4(secondList, myInterface);
                rv.setAdapter(adapter);
                rv.setLayoutManager(llm);
//                rv.addItemDecoration(new SimpleDividerItemDecoration(view.getContext().getResources().getDimensionPixelSize(R.dimen.photos_list_spacing),
//                        3));
            ...
            GridLayoutManager llm = new GridLayoutManager(view.getContext(), 2, GridLayoutManager.VERTICAL, false);
            try {
                adapter = new MainAdapter4(thirdList, myInterface);
                rv.setAdapter(adapter);
                rv.setLayoutManager(llm);
//                rv.addItemDecoration(new SimpleDividerItemDecoration(view.getContext().getResources().getDimensionPixelSize(R.dimen.photos_list_spacing),
//                        3));
